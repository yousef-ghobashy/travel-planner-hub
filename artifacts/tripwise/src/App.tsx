  ...
  return <div className="page"><PageHeading eyebrow="Getting there" title="Find your way in." description="Search your route with live provider links, then keep the planning decisions here." actions={<button className="btn btn-quiet" onClick={() => links.data?.flightSearchUrl && window.open(links.data.flightSearchUrl, '_blank')} data-testid="link-external-flight-search"><ExternalLink /> Compare live fares</button>} />
    <form className="card live-search" onSubmit={submitSearch}>
      ...
      <div className="search-fields">
        <label>From<input value={search.origin} onChange={event => setSearch({ ...search, origin: event.target.value })} placeholder="SFO or San Francisco" /></label>
        <label>To<input value={search.destination} onChange={event => setSearch({ ...search, destination: event.target.value })} placeholder="Barcelona or BCN" /></label>
    ...
    <div className="notice"><Info /><span>Tripwise uses live provider searches for current fares. This app does not invent prices, take payment, or confirm bookings.</span></div>
