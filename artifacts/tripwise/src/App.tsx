...
function Flights() {
  ...
  return <div className="page"><PageHeading eyebrow="Getting there" title="Find your way in." description="A few considered options for SFO → BCN, sorted by what matters beyond the lowest number." actions={<button className="btn btn-quiet" onClick={() => window.open('https://www.google.com/travel/flights', '_blank')} data-testid="link-external-flight-search">Compare live fares <ExternalLink /></button>} />
    <div className="notice"><Info /><span>Demo estimates for Apr 18–24, 2025. Prices and availability change — verify directly with the airline before purchase.</span></div>
    <div className="card" style={{ marginTop: 15 }}><div className="route-card"><div className="route-city"><span>From</span><strong>SFO</strong><span>San Francisco</span></div><div className="route-line" /><div className="route-city" style={{ textAlign: 'right' }}><span>To</span><strong>BCN</strong><span>Barcelona</span></div></div>{flights.map(flight => <div className={`flight-row ${saved.flight === flight.id ? 'selected' : ''}`} key={flight.id}><div className="flight-main"><div className="airline-mark">{flight.code}</div><div><div className="flight-times"><strong>{flight.out}</strong><span className="dash" /><strong>{flight.back}</strong></div><div className="flight-meta">{flight.airline} · {flight.duration} · {flight.stops}</div></div></div><div className="flight-price"><strong>{flight.price}</strong><span>roundtrip estimate</span></div><span className="option-tag">{flight.note}</span><button className={`btn ${saved.flight === flight.id ? 'btn-accent' : 'btn-quiet'}`} onClick={() => setSaved({ ...saved, flight: flight.id })} data-testid={`button-select-${flight.id}`}>{saved.flight === flight.id ? <><Check /> Selected</> : 'Select'}</button></div>)}</div>
    <div className="section-label"><h2>Good to know</h2></div><div className="grid grid-3"><div className="card card-pad"><div className="eyebrow">Timing</div><h3 style={{ margin: '9px 0 5px' }}>Arrive with daylight</h3><p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>The KLM option gives you an easy first afternoon to check in and walk the waterfront.</p></div><div className="card card-pad"><div className="eyebrow">Handoff</div><h3 style={{ margin: '9px 0 5px' }}>Book direct when ready</h3><p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>Tripwise will hand you off to the airline. It never handles payment or booking.</p></div><div className="card card-pad"><div className="eyebrow">Flexibility</div><h3 style={{ margin: '9px 0 5px' }}>Hold a backup</h3><p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>The Air France schedule is a useful alternative if arrival time matters more than fare.</p></div></div>
  </div>;
}

function Stays() {
  const [saved, setSaved] = useStored<Saved>('tripwise-selections', defaultSaved);
  return <div className="page"><PageHeading eyebrow="A place to land" title="Stay somewhere with a pulse." description="Three different ways into Barcelona. All are demo estimates, selected for feeling as much as location." actions={<button className="btn btn-primary" onClick={() => window.open('https://www.booking.com/city/es/barcelona.html', '_blank')} data-testid="link-external-stay-search">Search live availability <ExternalLink /></button>} />
    <div className="notice"><Info /><span>Availability and nightly prices are demo estimates for 6 nights. External links open provider searches; no reservation is made here.</span></div>
    <div className="option-grid grid" style={{ marginTop: 15 }}>{stays.map(stay => <div className={`card option-card ${saved.stay === stay.id ? 'selected' : ''}`} key={stay.id}><div className="option-card-body"><div className="option-top"><div><h3>{stay.name}</h3><p>{stay.area}</p></div><span className="option-tag">{saved.stay === stay.id ? 'Your choice' : 'Demo pick'}</span></div><div style={{ marginTop: 16, height: 84, borderRadius: 9, background: `linear-gradient(135deg, hsl(var(--primary) / .9), ${stay.id === 'stay-1' ? 'hsl(14 58% 62% / .8)' : stay.id === 'stay-2' ? 'hsl(204 34% 61% / .8)' : 'hsl(39 71% 65% / .75)'})`, position: 'relative', overflow: 'hidden' }}><div style={{ position: 'absolute', inset: '32px 0 0', background: 'hsl(var(--card) / .18)', clipPath: 'polygon(0 40%, 27% 0, 55% 45%, 74% 14%, 100% 55%, 100% 100%, 0 100%)' }} /><span style={{ position: 'absolute', right: 10, top: 10, color: 'hsl(40 38% 97% / .8)', fontSize: 10 }}>{stay.rating} / 10</span></div><p style={{ marginTop: 14 }}>{stay.note}</p><div className="option-details"><div><span>From</span><strong>{stay.price}<small className="muted"> / night</small></strong></div><div><span>Includes</span><strong>{stay.features[0]}</strong></div></div><div className="option-footer"><a className="external" href="https://www.booking.com" target="_blank" rel="noreferrer" data-testid={`link-provider-${stay.id}`}>Provider handoff <ExternalLink size={11} /></a><button className={`btn ${saved.stay === stay.id ? 'btn-accent' : 'btn-quiet'}`} onClick={() => setSaved({ ...saved, stay: stay.id })} data-testid={`button-select-${stay.id}`}>{saved.stay === stay.id ? <><Check /> Selected</> : 'Choose stay'}</button></div></div></div>)}</div>
  </div>;
}
...
function Itinerary() {
  ...
  return <div className="page"><PageHeading eyebrow="The shape of your days" title="A plan with breathing room." description="Three anchors, plenty of space. Your saved places will find their way here as the plan settles." actions={<button className="btn btn-accent" onClick={() => { const a = document.createElement('a'); const blob = new Blob(['Tripwise · Barcelona in full\\n\\nYour itinerary is ready to print.'], { type: 'text/html' }); a.href = URL.createObjectURL(blob); a.download = 'tripwise-itinerary.html'; a.click(); }} data-testid="button-export-itinerary"><FileDown /> Export itinerary</button>} />
    <div className="notice"><Info /><span>Route distances and costs are planning estimates. Venue links are provider handoffs — Tripwise does not book or confirm anything.</span></div>
...
function Budget() {
  const rows = [{ label: 'Flights', note: 'Roundtrip for 2 · KLM estimate', amount: '$1,636', pct: 48, icon: Plane }, { label: 'Accommodation', note: 'Casa Lirio · 6 nights', amount: '$1,284', pct: 72, icon: BedDouble }, { label: 'Food & coffee', note: 'A generous daily estimate', amount: '$382', pct: 34, icon: Utensils }, { label: 'Activities', note: 'Entries & experiences', amount: '$116', pct: 20, icon: Landmark }, { label: 'Local transport', note: 'Metro, airport transfer, walking', amount: '$62', pct: 12, icon: TrainFront }];
  return <div className="page"><PageHeading eyebrow="Keep the shape, not the stress" title="A budget that leaves room." description="A useful estimate for two travelers — not a promise, and definitely not a booking." actions={<button className="btn btn-quiet" onClick={() => { navigator.clipboard?.writeText('$3,480 estimated total · Barcelona'); }} data-testid="button-copy-budget"><Share2 /> Copy summary</button>} />
    <div className="budget-layout"><div className="budget-hero"><div className="eyebrow" style={{ color: 'hsl(var(--primary-foreground) / .65)' }}>Estimated trip total</div><div className="budget-total">$3,480</div><div className="budget-note">for 2 travelers · 6 nights · USD</div><div style={{ marginTop: 15 }}><div className="budget-progress-copy"><span>Planning estimate used</span><span>$5,100 comfort ceiling</span></div><div className="budget-track" style={{ marginTop: 8 }}><span /></div></div><div className="notice" style={{ marginTop: 15, background: 'hsl(var(--primary-foreground) / .09)', color: 'hsl(var(--primary-foreground) / .78)' }}><Info /><span>Prices are labeled demo estimates. Verify current fares, cancellation terms, and exchange rates with each provider.</span></div></div><div className="card card-pad">{rows.map(row => { const Icon = row.icon; return <div className="budget-row" key={row.label}><div className="budget-icon"><Icon /></div><div className="budget-row-main"><strong>{row.label}</strong><span>{row.note}</span><div className="bar-track"><span style={{ width: `${row.pct}%` }} /></div></div><div className="budget-amount">{row.amount}</div></div>; })}</div></div>
    <div className="section-label"><h2>Daily rhythm</h2><span className="eyebrow">demo estimate</span></div><div className="grid grid-3">{['Arrival · $210', 'Old city · $104', 'Gaudí day · $178'].map((x, i) => <div className="card card-pad" key={x}><div className="eyebrow">Day {i + 1}</div><div className="serif" style={{ fontSize: 25, marginTop: 8 }}>{x.split(' · ')[1]}</div><div className="muted" style={{ fontSize: 11, marginTop: 3 }}>{x.split(' · ')[0]}</div></div>)}</div>
  </div>;
}
...
function Trips() {
  const [, setLocation] = useLocation();
  return <div className="page"><PageHeading eyebrow="Your library" title="My trips" description="Keep the plans worth returning to. Tripwise saves your choices locally in this demo workspace." actions={<button className="btn btn-primary" onClick={() => setLocation('/')} data-testid="button-new-trip"><Plus /> Start a new trip</button>} />
